...
//		public override string SiteRootPath { get; set; }
		public bool UseSiteData { get; set; }
...
//			Info.Initialize();
		...
		{
			MainWindow.Instance.MsgLeftLine("");
			MainWindow.Instance.MsgLeftLine("at ctor AppSettings| UseMachData", Info.UseMachData.ToString());

			if (Info.UseMachData)
			{
				MachAdmin = MachSettings.Admin;

				MachAdmin.Initialize();
	...
	#region machine settings root class
		...
		public static class MachSettings
		{
			...
			static MachSettings()
			{
				...
				MainWindow.Instance.MsgLeftLine("at ctor MachSettings|     path", Admin.Info.SettingPath);
				MainWindow.Instance.MsgLeftLine("at ctor MachSettings| filename", Admin.Info.FileName);
			}
